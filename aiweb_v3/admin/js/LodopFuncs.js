...
function needCLodop() {
    try {
        if(getCookieLodop('lodopType')=='Clodop') return true;
        if(getCookieLodop('lodopType')=='lodop') return false;
...
function getLodop(oOBJECT, oEMBED) {
    ...
    try {
        ...
        if (needCLodop()) {
            ...
            if (!LODOP) {
                ...
                if (CLODOP.CVERSION < "3.0.5.6") {
                    ...
                    //return false;//解决页面只要有返回无论是否正常就会继续多次调用会导致出现很多个升级提示，所以直接返回false，原程序没有此返回代码，可删除
                }
        ...
        if (LODOP.VERSION < "6.2.2.4") {
            ...
            return false;//现有页面只要有返回无论是否正常就会继续多次调用页面会出现很多个升级提示，所以直接返回false
            //return LODOP;
        }
        //===如下空白位置适合调用统一功能(如注册语句、语言选择等):===
        LODOP.SET_LICENSES("航天国政信息技术（北京）有限公司","ECC883BD6642965613613DF832DBB70B","","");
        //=======================================================
        return LODOP;
    } catch (err) {
